...
#agrego pruebas para el logaritmo en base 10 - Casos de error
    ...
    #Se agregan pruebas para la multiplicación con permisos - Casos de éxito
    ...
    #Se agregan pruebas para la suma con permisos - Casos de éxito
    @patch('app.util.validate_permissions', side_effect=mocked_validation, create=True)
    def test_add_method_returns_correct_result_with_permissions(self, _validate_permissions):
        self.assertEqual(4, self.calc.add(2, 2))
        self.assertEqual(0, self.calc.add(2, -2))
        self.assertEqual(0, self.calc.add(-2, 2))
        self.assertEqual(1, self.calc.add(1, 0))
    ...
    #Se agregan pruebas para la resta con permisos - Casos de éxito
    @patch('app.util.validate_permissions', side_effect=mocked_validation, create=True)
    def test_substract_method_returns_correct_result_with_permissions(self, _validate_permissions):
        self.assertEqual(0, self.calc.substract(2, 2))
        self.assertEqual(16, self.calc.substract(10, -6))
        self.assertEqual(-3, self.calc.substract(-2, 1))
        self.assertEqual(1, self.calc.substract(1, 0))

    #se agregan pruebas para la resta con permisos - Casos de error
    @patch('app.util.validate_permissions', return_value=mocked_validation, create=True)
    def test_substract_method_fails_with_invalid_permissions(self, _validate_permissions):
        _validate_permissions.return_value = False
        self.assertRaises(Exception, self.calc.substract, 2, 2)

    #Se agregan pruebas para la división con permisos - Casos de éxito
    @patch('app.util.validate_permissions', side_effect=mocked_validation, create=True)
    def test_divide_method_returns_correct_result_with_permissions(self, _validate_permissions):
        self.assertEqual(1, self.calc.divide(2, 2))
        self.assertEqual(1.5, self.calc.divide(3, 2))

    #se agregan pruebas para la división con permisos - Casos de error
    @patch('app.util.validate_permissions', return_value=mocked_validation, create=True)
    def test_divide_method_fails_with_invalid_permissions(self, _validate_permissions):
        _validate_permissions.return_value = False
        self.assertRaises(Exception, self.calc.divide, 2, 2)

    #Se agregan pruebas para la potenciación con permisos - Casos de éxito
    @patch('app.util.validate_permissions', side_effect=mocked_validation, create=True)
    def test_power_method_returns_correct_result_with_permissions(self, _validate_permissions):
        self.assertEqual(4, self.calc.power(2, 2))
        self.assertEqual(1, self.calc.power(3, 0))
        self.assertEqual(0.25, self.calc.power(2, -2))
        self.assertEqual(0, self.calc.power(0, 2))
    ...
    #se agregan pruebas para la raíz cuadrada con permisos - Casos de error
    @patch('app.util.validate_permissions', return_value=mocked_validation, create=True)
    def test_sqrt_method_fails_with_invalid_permissions(self, _validate_permissions):
        _validate_permissions.return_value = False
        self.assertRaises(Exception, self.calc.sqrt, 4)

    #Se agregan pruebas para el logaritmo en base 10 con permisos - Casos de éxito
    @patch('app.util.validate_permissions', side_effect=mocked_validation, create=True)
    def test_log10_method_returns_correct_result_with_permissions(self, _validate_permissions):
        self.assertEqual(1, self.calc.log10(10))
        self.assertEqual(0, self.calc.log10(1))
        self.assertEqual(2, self.calc.log10(100))

    #se agregan pruebas para el logaritmo en base 10 con permisos - Casos de error
    @patch('app.util.validate_permissions', return_value=mocked_validation, create=True)
    def test_log10_method_fails_with_invalid_permissions(self, _validate_permissions):
        _validate_permissions.return_value = False
        self.assertRaises(Exception, self.calc.log10, 10)
