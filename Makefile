...
server:
	docker run --rm --volume $(CURDIR):/opt/calc --name apiserver --env PYTHONPATH=/opt/calc --env FLASK_APP=app/api.py -p 5000:5000 -w /opt/calc calculator-app:latest flask run --host=0.0.0.0

interactive:
	docker run -ti --rm --volume $(CURDIR):/opt/calc --env PYTHONPATH=/opt/calc  -w /opt/calc calculator-app:latest bash

test-unit:
	-docker run --rm --volume $(CURDIR):/opt/calc --env PYTHONPATH=/opt/calc -w /opt/calc calculator-app:latest pytest --cov --cov-report=xml:results/coverage.xml --cov-report=html:results/coverage --junit-xml=results/unit_result.xml -m unit
...
test-api:
	...
	docker run -d --rm --volume $(CURDIR):/opt/calc --network calc-test-api --env PYTHONPATH=/opt/calc --name apiserver --env FLASK_APP=app/api.py -p 5000:5000 -w /opt/calc calculator-app:latest flask run --host=0.0.0.0
	-docker run --rm --volume $(CURDIR):/opt/calc --network calc-test-api --env PYTHONPATH=/opt/calc --env BASE_URL=http://apiserver:5000/ -w /opt/calc calculator-app:latest pytest --junit-xml=results/api_result.xml -m api 
...
test-e2e:
	...
	docker run -d --rm --volume $(CURDIR):/opt/calc --network calc-test-e2e --env PYTHONPATH=/opt/calc --name apiserver --env FLASK_APP=app/api.py -p 5000:5000 -w /opt/calc calculator-app:latest flask run --host=0.0.0.0
	...
	-docker run --rm --volume $(CURDIR)/test/e2e/cypress.json:/cypress.json --volume $(CURDIR)/test/e2e/cypress:/cypress --volume $(CURDIR)/results:/results  --network calc-test-e2e cypress/included:4.9.0 --browser chrome
	docker rm --force apiserver
	docker rm --force calc-web
...
test-e2e-wiremock:
	...
	-docker run --rm --volume $(CURDIR)/test/e2e/cypress.json:/cypress.json --volume $(CURDIR)/test/e2e/cypress:/cypress --volume $(CURDIR)/results:/results --network calc-test-e2e-wiremock cypress/included:4.9.0 --browser chrome
	docker rm --force apiwiremock
	docker rm --force calc-web
	docker run --rm --volume $(CURDIR):/opt/calc --env PYTHONPATH=/opt/calc -w /opt/calc calculator-app:latest junit2html results/cypress_result.xml results/cypress_result.html
...
stop-wiremock:
	-docker stop calculator-wiremock
...
zap-scan:
	...
	docker run -d --rm --network calc-test-zap --volume $(CURDIR):/opt/calc --name apiserver --network-alias apiserver --env PYTHONPATH=/opt/calc --env FLASK_APP=app/api.py -p 5000:5000 -w /opt/calc calculator-app:latest flask run --host=0.0.0.0
	...
	sleep 10
	-docker run --rm --volume $(CURDIR):/opt/calc --network calc-test-zap --env PYTHONPATH=/opt/calc --env ZAP_API_KEY=$(ZAP_API_KEY) --env ZAP_API_URL=$(ZAP_API_URL) --env TARGET_URL=$(ZAP_TARGET_URL) -w /opt/calc calculator-app:latest pytest --junit-xml=results/sec_result.xml -m security 
	...
	-docker stop apiserver
	-docker stop calc-web
	-docker stop zap-node
	-docker network rm calc-test-zap
...
start-jmeter-record:
	...
	docker run -d --rm --network calc-test-jmeter --volume $(CURDIR):/opt/calc --name apiserver --network-alias apiserver --env PYTHONPATH=/opt/calc --env FLASK_APP=app/api.py -p 5000:5000 -w /opt/calc calculator-app:latest flask run --host=0.0.0.0
...
stop-jmeter-record:
	-docker stop apiserver
	-docker stop calc-web
	-docker network rm calc-test-jmeter
